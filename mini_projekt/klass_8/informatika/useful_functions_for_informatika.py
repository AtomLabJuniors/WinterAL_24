...
from klass_8.informatika.list_obj_informatika import INFORMATIKA
...
if __name__ == "__main__":
    quantity_urls = list(INFORMATIKA.keys())
    for i in quantity_urls:
        if INFORMATIKA[str(i)]["url"]:
            print(f'{i}) {INFORMATIKA[str(i)]["about"]}')

    book = str(int(input("Enter the appropriate textbook: ")))
    exer = int(input("Enter the number of exercise: "))
    ans = download_gdz(INFORMATIKA[book]["url"], exer)
    print(ans)
