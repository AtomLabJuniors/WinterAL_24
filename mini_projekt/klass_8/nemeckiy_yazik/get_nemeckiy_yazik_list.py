from gdz import GDZ
from chec_url_nemeckiy_yazik_on_existence import get_url_if_url_is_true
...
for book in GDZ().books:
    ...
    n += 1
    if (
        (name_textbook in book.title.lower())
        and ("част" not in book.title.lower())
        and (klass in book.title)
    ):
