...
from klass_7.nemeckiy_yazik.list_obj_nemeckiy_yazik import NEMECKIY_YAZIK
...
if __name__ == "__main__":
    quantity_urls = list(NEMECKIY_YAZIK.keys())
    for i in quantity_urls:
        if NEMECKIY_YAZIK[str(i)]["url"]:
            print(f'{i}) {NEMECKIY_YAZIK[str(i)]["about"]}')

    book = str(int(input("Enter the appropriate textbook: ")))
    exer = int(input("Enter the number of exercise: "))
    ans = download_gdz(NEMECKIY_YAZIK[book]["url"], exer)
    print(ans)
