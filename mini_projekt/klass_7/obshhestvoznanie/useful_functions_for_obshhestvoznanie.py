...
from klass_7.obshhestvoznanie.list_obj_obshhestvoznanie import OBSHHESTVOZNANIE
...
if __name__ == "__main__":
    quantity_urls = list(OBSHHESTVOZNANIE.keys())
    for i in quantity_urls:
        if OBSHHESTVOZNANIE[str(i)]["url"]:
            print(f'{i}) {OBSHHESTVOZNANIE[str(i)]["about"]}')

    book = str(int(input("Enter the appropriate textbook: ")))
    exer = int(input("Enter the number of exercise: "))
    ans = download_gdz(OBSHHESTVOZNANIE[book]["url"], exer)
    print(ans)
